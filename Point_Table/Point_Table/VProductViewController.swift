...
class VProductViewController: UIViewController,UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
...
//        collectionV.register(Header1.self, forSupplementaryViewOfKind:
//                  UICollectionView.elementKindSectionFooter, withReuseIdentifier: headerId)
//              collectionV.register(Header1.self, forCellWithReuseIdentifier: headerId)
//
        ...
        let tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(VProductViewController.closeFilter(_:)))
...
//               bottomRefreshController.addTarget(self, action: #selector(VProductViewController.refreshBottom), for: .valueChanged)
//
//               collectionV.bottomRefreshControl = bottomRefreshController
    ...
    func SetupUI() -> Void {
       ...
       btnUpdateQty.titleLabel?.font =  UIFont(name: Font_Semibold, size: 17)
        ...
        let button = UIButton(frame: CGRect(x: 0, y: 0, width: tblProductSearch.frame.size.width, height: 50))
        button.setTitle("View All Products", for: .normal)
        button.titleLabel?.font =  UIFont(name: Font_Semibold, size: 17)
    ...
    func getAllBrand() -> Void {
        if Reachability.isConnectedToNetwork() {
            ...
            param["ProductSearchKeyword"] = strSearchKey
            ...
            if Reachability.isConnectedToNetwork() {
                   ...
                   // param["IsDisplayPromotionOnly"] = true
                    ...
                    param["BrandId"] = strBrandId
                       ...
                       param["PageSize"] = pageSize
                        param["ProductSearchKeyword"] = strSearchKey
                       APIManager.requestPostJsonEncoding( .getallproductweb , isLoading: (self.isPageLoading ? false : true), params: param, headers: [:],success: { (JSONResponse)  -> Void in
...
//        }
    ...
    /*func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        let header = collectionView.dequeueReusableCell(withReuseIdentifier: headerId, for: indexPath) as! Header1
...
extension VProductViewController :UITableViewDataSource {
...
extension VProductViewController :UITableViewDelegate {
...
extension VProductViewController : UITextFieldDelegate {
...
extension VProductViewController  {
